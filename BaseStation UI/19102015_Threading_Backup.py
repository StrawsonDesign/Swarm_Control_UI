...
class otherdrones(threading.Thread):
    def __init__(self,master):
        ...
        #self.updateActiveDrones(droneFrame)threading.Thread
...
class logger(threading.Thread):
    def __init__(self,listeningThread):
        ...
        sleep(0.5) # sleep 1 second while initailizing so that buffer can stream intial values
...
def UDP():
    listeningThread=listener(6) # sizeOfRingBuffer
    listeningThread.setDaemon(False) # exit UI even if some listening is going on
