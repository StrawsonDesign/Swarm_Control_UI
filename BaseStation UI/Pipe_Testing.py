...
def UDP(listen_conn, log_conn):
    UDPlistenThread=listener(10,listen_conn) # sizeOfRingBuffer
    UDPlistenThread.setDaemon(True)

    UDPlogThread=logger(log_conn)
...
def main():
	...
	log_parent_conn, listen_parent_conn = multiprocessing.Pipe()
	log_child_conn, listen_child_conn = multiprocessing.Pipe()
	udpProcess=multiprocessing.Process(name='UDP Process', target=UDP, args=(listen_child_conn, log_child_conn))
